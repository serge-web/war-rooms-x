...
export const renderObjectContent = (obj: unknown, level: number = 0): React.ReactNode => {
  ...
  if (typeof obj === 'string') {
    return <span className='message-value message-string'>'{obj}'</span>
  } 
  ...
  return (
    <div className='message-object' style={{ marginLeft: `${level * 12}px` }}>
      {entries.map(([key, value]) => (
        <div key={key} className='message-object-item'>
          <span className='message-label'>{key}:</span> {renderObjectContent(value, level + 1)}
        </div>
      ))}
    </div>
  )
